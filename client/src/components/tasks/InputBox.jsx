...
const InputBox = ({ initialText, initialRemTime, initialImportance, initialUrgency, initialPriority, initialDesc, initialColor, onAfterSubmit }) => {
  ...
  const todayMinDate = getLocalDateTime(new Date()).split('T')[0];
  const minTimeForNow = getLocalDateTime(new Date(Date.now() + 60000)).split('T')[1];
  ...
  const handleSubmit = (e) => {
    ...
    store.list = [...snap.list, { id, title, status: true, remTime: remTimeCombined, importance, urgency, priority, color }];
    setText("");
