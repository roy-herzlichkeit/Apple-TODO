...
const List = ({ list, onEdit }) => {
  ...
  return (
    <div>
      <h3>PENDING TASKS:</h3>
      {
        list.map(item => item.status && <Item key={item.id} title={item.title} priority={item.priority} remTime={item.remTime} handleDeletion={() => handleDeletion(item.id)} handleToggle={() => handleToggle(item.id)} handleEdit={() => handleEdit(item.id)} />)
      }
